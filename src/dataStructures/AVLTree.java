...
public class AVLTree<K extends Comparable<K>, V> extends AdvancedBSTree<K, V> {
	...
	/**
	 * Every ancestor of the new leaf is stored in the stack, which is not empty.
	 * 
	 * @param path
	 *            - Stack of PathStep objects containing all ancestors of the
	 *            inserted node
	 */
	...
	/**
	 * Every ancestor of node is stored in the stack, which is not empty. height(
	 * node.getLeft() ) - height( node.getRight() ) = 2.
	 * 
	 * @param node
	 *            - root of subtree to balance
	 * @param path
	 *            - Stack of PathStep objects containing all ancestors of node
	 */
	...
	protected void reorganizeRem(Stack<PathStep<K, V>> path) {
		...
		while (remove && parent != null) {

			if (!lastStep.isLeftChild) {

				// the parent decreases on the right side
				switch (parent.getBalance()) {
				case 'L':
				...
				// the parent decreases on the left side
				switch (parent.getBalance()) {
				case 'L':
					parent.setBalance('E');
					break;
	...
	/**
	 * Performs a single left rotation rooted at theRoot, when the balance factor of
	 * its leftChild is 'L'. Every ancestor of theRoot is stored in the stack, which
	 * is not empty. height( node.getLeft() ) - height( node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 ...
	 * @param path
	 *            - Stack of PathStep objects containing all ancestors of theRoot
	 */
	...
	/**
	 * Performs a single left rotation rooted at theRoot, when the balance factor of
	 * its leftChild is 'E'. Every ancestor of theRoot is stored in the stack, which
	 * is not empty. height( node.getLeft() ) - height( node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	...
	/**
	 * Performs a single right rotation rooted at theRoot, when the balance factor
	 * of its rightChild is 'R'. Every ancestor of theRoot is stored in the stack,
	 * which is not empty. height( node.getRight() ) - height( node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param rightChild
	...
	/**
	 * Performs a single right rotation rooted at theRoot, when the balance factor
	 * of its rightChild is 'E'. Every ancestor of theRoot is stored in the stack,
	 * which is not empty. height( node.getRight() ) - height( node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	...
	/**
	 * Performs a double left rotation rooted at theRoot. Every ancestor of theRoot
	 * is stored in the stack, which is not empty. height( node.getLeft() ) -
	 * height( node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param leftChild
	...
	/**
	 * Performs a double right rotation rooted at theRoot. Every ancestor of theRoot
	 * is stored in the stack, which is not empty. height( node.getRight() ) -
	 * height( node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param rightChild
	 ...
	 *            inserted node
	 */
	private void rebalanceRemRight(AVLNode<K, V> parent, Stack<PathStep<K, V>> path) {
	 ...
	 *            inserted node
	 */
	private void rebalanceRemLeft(AVLNode<K, V> parent, Stack<PathStep<K, V>> path) {
